...
- **Phase 3**: covers risk #4. Uses Vitest for Angular component tests, not Karma — `tsconfig.spec.json` already declares `vitest/globals` and no Karma package exists in the repo. The `submitting`-flag / double-submit mutant class on `alert-form.ts` (the Alert Form counterpart of the Phase 5 / #113 admin work) was closed as a follow-up in issue #114, together with the previously-uncovered `messageFor` error-map branches. Its scoped Stryker pass is line-range-scoped to `alert-form.ts:146-181` (`onSubmit` + `messageFor`): instrumenting the whole file wraps the `fb.nonNullable.group({...})` initializer and breaks `strictTemplates` on `alert-form.html`. The `alert-form.html:85` `[disabled]` binding is covered by a deliberate-break check, not Stryker (the Angular `command` runner does not mutate templates). The identical `submitting`-flag / double-submit class on `register.ts` — plus its previously-uncovered inline error-handler branches (`err instanceof HttpErrorResponse && err.status === 409` vs the generic message) — was closed the same way in issue #115: line-range Stryker scope `register.ts:27-53` for the same `fb.nonNullable.group` / `strictTemplates` reason, and the `register.html:31` `[disabled]` binding covered by a deliberate-break check. `login.ts` — the last auth/form component with no spec at all — got its first component coverage (validator gates, happy path with navigation assertion, error message) plus the same `submitting` / double-submit guard treatment in issue #116: line-range Stryker scope `login.ts:26-43` (whole-file instrumentation briefly appears to compile while no spec imports the component, but breaks `strictTemplates` on `login.html` once one does), 12/13 mutants killed with `errorMessage.set(null)` the one accepted survivor (→ #110), and the `login.html:31` `[disabled]` binding covered by a deliberate-break check. Issue #110 then ran the **broad** whole-component Stryker sweep across all three (`alert-form.ts` 72→93%, `register.ts` 84→92%, `login.ts` 92→100%): multi-range `--mutate` skipping the `fb.nonNullable.group` initializer (comma-separated ranges, e.g. `alert-form.ts:1-60,75-182` — same `strictTemplates` reason as the line-range scopes above), adding success-path coverage (`dialogRef.close(true)` / `navigateByUrl('/')` / submitted-payload shape — no prior test emitted a successful request), the error-reset facet, and `alert-form` display-helper + `valueChanges`-cascade coverage. `login.ts:32` is now killed; the residual survivors are documented equivalents (defensive `{ optional: true }` on a dialog-only component, `typeof x === 'number'` guards redundant with `> 0` / `Number.isFinite`, `alert-form.ts:168` `instanceof` carried from #114, `register.ts` `emailError.set(null)` / `markAsTouched()` — both masked by mat-form-field's own errorState gate). The Angular `command` runner does not fail reliably on mutant-introduced TS type errors (warm incremental cache), so `&&`↔`||` mutations that break null-narrowing show as survived — treated as equivalent.
...
| Angular component tests | Vitest (not Karma — see §3 Phase 3 scope notes) | latest | `tsconfig.spec.json` already declares `vitest/globals`; no separate Angular-specific test runner (Karma) was introduced. Three component test files (`alert-form.spec.ts`, `register.spec.ts`, `login.spec.ts`), each hardened by a scoped Stryker pass — #114/#115/#116 for the submit-guard class, #110 for the broad whole-component sweep. Admin panel components covered separately (#113 / §3 Phase 5, risk #8) |
