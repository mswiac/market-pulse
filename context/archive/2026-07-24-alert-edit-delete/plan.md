...
**Contract**: Standalone component; injects `MAT_DIALOG_DATA` typed as `{ instrument: string; alertType: string; threshold: string }` — all three fields are pre-formatted display strings built by the caller (`AlertList`) rather than duplicated in this component. Dialog renders them as a labeled `<dl>` (three `<dt>`/`<dd>` pairs, bolded labels), reusing the exact "Walor"/"Typ alertu"/"Próg" column labels already established by `AlertList`'s sortable header (`alertList.sort.instrument`/`alertList.sort.alertType`/`alertList.sort.threshold` i18n ids, reused verbatim), plus a body line stating the action cannot be undone.

**Addendum (post-implementation UI iteration, 2026-07-24)**: The original contract specified a single pre-formatted `summary: string` rendered as one flat line (e.g. `"VIX · Price · 35.00"`). Live user feedback during Phase 4 found this illegible — the type label and the bare trailing number blended together with no way to tell what the number represented. Per Material Design 3 guidance (a dialog's content area should be either prose or a list of items, not a hybrid), the contract above replaces the flat string with three explicitly labeled fields. `AlertList.deleteAlert()` also introduces `ALERT_TYPE_SHORT_LABELS` (`'PRICE'` → "Cena" reusing `alertForm.alertType.price`, `'RSI'` → `"RSI"`) instead of the list's own `alertTypeLabel()` (which returns the longer "Próg cenowy"/"Próg RSI" column-context labels) — the short form avoids redundantly repeating "próg" once the `<dt>` label already says it.

`<mat-dialog-actions>` has a `mat-dialog-close` Cancel button with `cdkFocusInitial`, and a `color="warn"` Delete button that closes the dialog with `true`.
...
- [x] 3.1 Typecheck passes: `npm run typecheck` — c72802c
- [x] 3.2 `npx ng extract-i18n --output-path src/locale` exits 0 and includes every new id from this phase — c72802c
- [x] 3.3 Production build succeeds: `npm run build` — c72802c
