...
**Addendum (post-implementation UI polish, added after user feedback rounds, confirmed via impl-review 2026-07-19)**: The shipped list goes beyond this original contract — a sortable column-header row (Walor/Typ alertu/Próg, click to sort, click again to reverse, arrow icon indicator), CSS Grid column alignment so values line up regardless of instrument-name length, M3 color differentiation between the panel header and body (`--mat-sys-secondary-container` vs `--mat-sys-surface-container-low`), threshold formatted to 2 decimals via `DecimalPipe`, and a load-error state ("Nie udało się wczytać alertów.") when `list()` fails. Row-level Walor/Typ alertu/Próg labels were removed from the collapsed summary (now carried by the header row instead); the expanded detail retains its labels and was reordered to Aktualna cena → Aktualne RSI → E-mail powiadomień → Ostatnia edycja.
...
**Addendum (post-implementation, confirmed via impl-review 2026-07-19)**: The error-mapping mechanism deviated from the `setErrors`/`markAsTouched()` plan text — actual implementation uses a `formError` signal + banner (mirroring `login.ts`'s pattern) since these errors aren't attributable to one specific field; behavior (blocking, surfacing, never leaving the dialog stuck) is unchanged. Additional UI-polish fixes: the `threshold` control resets to `null` (not just re-validates) when `alertType` changes; `onThresholdBlur` reformats the visible input text to 2 decimals without touching the control's numeric value; all four `mat-form-field`s use `subscriptSizing="dynamic"` plus `margin-bottom` on `.full-width` so a wrapped two-line validation error doesn't overlap the next field.
...
**Addendum (post-implementation UI polish, confirmed via impl-review 2026-07-19)**: The trigger button was changed to a fixed-position extended FAB (`<button matFab extended color="primary" class="new-alert-fab"><mat-icon>add</mat-icon>Nowy alert</button>`) per Material 3 guidance (FAB is the correct pattern when "add" is the screen's primary action), opened with `this.dialog.open(AlertForm, { width: '32rem' })`. The welcome card and alerts section were both set to `max-width: 48rem` for visual consistency.
