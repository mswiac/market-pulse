...
- **Phase 3**: covers risk #4. Uses Vitest for Angular component tests, not Karma — `tsconfig.spec.json` already declares `vitest/globals` and no Karma package exists in the repo. The `submitting`-flag / double-submit mutant class on `alert-form.ts` (the Alert Form counterpart of the Phase 5 / #113 admin work) was closed as a follow-up in issue #114, together with the previously-uncovered `messageFor` error-map branches. Its scoped Stryker pass is line-range-scoped to `alert-form.ts:146-181` (`onSubmit` + `messageFor`): instrumenting the whole file wraps the `fb.nonNullable.group({...})` initializer and breaks `strictTemplates` on `alert-form.html`. The `alert-form.html:85` `[disabled]` binding is covered by a deliberate-break check, not Stryker (the Angular `command` runner does not mutate templates).
