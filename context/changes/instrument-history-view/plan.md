...
*Revised several times during implementation*: the plan originally specified a `mat-menu` dropdown anchored to a toolbar icon button, scoped to `Home` only. Manual testing surfaced two problems: (1) the trigger was easy to miss next to the logout button, and (2) because the menu lived inside `Home`, navigating to `/history` lost the toolbar entirely — no logout button, no user info, no way back to alerts. Both were fixed by (a) lifting the toolbar + nav out of `Home` entirely into a new shared `Shell` component that wraps both pages via nested routing, so the chrome persists across navigation instead of being re-mounted per page, and (b) iterating on the nav itself — a permanently-open `mat-sidenav` (`mode="side"`) → a two-item flat list → (per user direction, so a future "History" grouping could hold more than one destination) a single `mat-nav-list` where "Alerts" is a direct link and "History" is a plain `<button>` + `historyExpanded` signal toggle that reveals a nested "Instruments" link, rather than a separate `mat-expansion-panel` (which read as a visually disconnected floating card instead of one cohesive menu).

**Contract**: `Shell` (`src/app/core/shell/shell.ts`) is a new standalone component: `imports: [MatToolbarModule, MatButtonModule, MatIconModule, MatSidenavModule, MatListModule, RouterOutlet, RouterLink, RouterLinkActive]`; owns `AuthService.currentUser` and `onLogout()` (moved out of `Home`). Its template renders the toolbar (unchanged content) followed by `<mat-sidenav-container>` with a permanently-open `<mat-sidenav mode="side" [opened]="true" position="start">` containing a single `<mat-nav-list>`: a direct `<a mat-list-item routerLink="/">` for "Alerts" (with `routerLinkActive` highlighting), and a `<button mat-list-item>` for "History" that toggles a `historyExpanded` signal (`[attr.aria-expanded]` reflects state; initial value is derived from `router.url.startsWith('/history')` so a direct navigation/refresh onto `/history` lands with the section already expanded, not collapsed) — when expanded, a nested `<a mat-list-item routerLink="/history">` labeled "Instruments" appears underneath. `<mat-sidenav-content>` holds a bare `<router-outlet />`. `app.routes.ts` nests `Home` and `InstrumentHistory` as children of a `Shell` parent route (see item 3). `Home` no longer owns any toolbar/nav/logout code — it now renders only the welcome card, alert list, and "New alert" FAB.
